...
export const SERVER_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => ServerCompleteComponent),
    multi: true
};
...
@Component({
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <template let-server>
                <server-line [server]="server"></server-line>
            </template>
        </p-autoComplete>
	`,
	selector: 'server-auto-complete',
    providers: [SERVER_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class ServerCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Server = null;

    private suggestions : Server[] = [];
    ...
    complete(event:any) {
        this.serverService.complete(event.query).
            subscribe(
